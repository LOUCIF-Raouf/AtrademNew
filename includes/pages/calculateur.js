$(document).ready(function () {
    ...
    function updateMontage() {
        ...
        if (articleTab.length === 0) {
            ...
            $('#listObjMontage').children().remove();
            articleTab.forEach((article, i) => {
                ...
                checkBoxLabel.innerText = article.name + ' X' + article.quantity;
                ...
                checkBoxInput.addEventListener('change', e => {
                    ...
                    if (toggle === false) {
                        Cart.addItem({
                            ...
                            price: price,
                            quantity: 1,
                            label: 'Montage : ' + article.name + ' X' + article.quantity,
                        });
                        toggle = true;
                    } else {
                        Cart.addItem({
    ...
    function addArticle(articleId, articleName, articleSize, articlePrice) { //ajout d'un article
    ...
    /*   selects.forEach(select => {
        select.addEventListener('click', e => {
            e.preventDefault();
            console.log(e);
            addArticle(button.dataset.article, button.dataset.size, button.dataset.price); // fonction ajouter article param: nom de l'article
        });
    });
 */

    /* let question1 = document.getElementById("cartonQuestion-1");
    let question2 = document.getElementById("cartonQuestion-2");
    let question3 = document.getElementById("cartonQuestion-3");
    ...
    $("#basic-assurance").click(e => {
        ...
        document.getElementById('default-assurance').style.transition = "filter 0.3s";
    ...
    buttons.forEach(button => {
        ...
        button.addEventListener('click', e => {
            e.preventDefault();
            addArticle(button.dataset.id, button.dataset.article, button.dataset.size, button.dataset.price); // fonction ajouter article param: nom de l'article
        });
    });
