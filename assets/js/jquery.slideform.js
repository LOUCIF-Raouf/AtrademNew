...
(function ($) {
    $.fn.slideform = function (userOptions, args) {
         ...
         */
        $form
            .find('input[type=radio] + span, input[type=checkbox] + span')
            .on('click', function () {
        ...
        function init() {
            $form.addClass('slideform-form').addClass('slideform-initialized');

            // Adding the track.
            $track = $('<div />', {
                class: 'slideform-track'
            }).insertBefore($form.find('.slideform-slide').eq(0));

            // Adding the wrapper.
            $wrapper = $('<div />', {
                    class: 'slideform-wrapper'
                })
                .appendTo($track)
            ...
            $form.find('.slideform-slide').each(function (k, v) {
                ...
                if (k < $slides.length - 2) {
                    ...
                    $(this)
                        .find('.slideform-group')
                        .append(
                            '<button type="submit" class="slideform-btn slideform-btn-submit"><i class="icon-check"></i>&nbsp;&nbsp;' +
        ...
        $form.goTo = function (slide) {
            ...
            /*  $slides.filter('.active').next().animate(
              {
                scrollTop: 0,
              },
              'fast'
        ...
        function updateButtonsStatus(slideNumber) {
            $form
                .find('.slideform-btn-next')
                .prop('disabled', slideNumber == $slides.length - 1);
            $form.find('.slideform-btn-prev').prop('disabled', slideNumber == 0);

            $form
                .find('footer .slideform-btn-next')
                .prop('disabled', slideNumber >= $max);
        }
        ...
        function triggerConditions() {
            var input = {};
            var array = $form.serializeArray().map(function (i) {
